...
public class RenamePlan implements Plan {
   ...
   // The parser does not seem to check if a field name has been renamed
   
   public RenamePlan(Plan p, Collection<String> fieldlist) {
      this.p = p;
      for (String fldname : fieldlist) {
         if (fldname.contains(" as ")) {
             String[] renameSplit = fldname.split(" as ");
             String rename = renameSplit[renameSplit.length-1];
             schema.add(rename, p.schema());
         }
         else {
